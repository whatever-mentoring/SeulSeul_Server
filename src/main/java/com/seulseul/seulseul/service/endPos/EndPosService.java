...
public class EndPosService {
	...
	@Transactional
	public EndPosResDto saveEndPos(EndPosDto form, User user) {
        ...
        */
		if (baseRouteRepository.findByUser(user).isEmpty()) {
			...
			return new EndPosResDto(endPos.getId(), baseRoute.getId(), endPos.getEndX(), endPos.getEndY(),
				endPos.getEndNickName()
        ...
        */
		...
		return new EndPosResDto(endPos.getId(), baseRoute.get().getId(), endPos.getEndX(), endPos.getEndY(),
			endPos.getEndNickName()
			, endPos.getRoadNameAddress(), endPos.getJibunAddress());
	}

	// 사용자의 모든 endPos(목적지) 가져오기
	public List<EndPos> findAllEndPos(User user) {
		List<EndPos> endPosList = endPosRepository.findAllByUser(user);
		return endPosList;
	}

	// 선택한 endPos(목적지) 가져오기
	public EndPosResDto findEndPos(Long id, User user) throws IOException, ParseException {
