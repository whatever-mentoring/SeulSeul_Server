...
import static com.seulseul.seulseul.dto.android.RouteDetailWrapDto.*;
...
public class FcmService {
	...
	public void schedule(BaseRoute baseRoute) throws JsonProcessingException {
		...
		String cronExpression = convertToCron(timeList[0], Math.toIntExact(baseRoute.getAlarm().getAlarmTime()),
			baseRoute.getAlarm().getAlarmTerm()); // DB에서 alarmTerm을 가져와서 cron 표현식으로 변환
		...
		Runnable task = () -> {
			try {
				...
				if (latestBaseRoute != null && latestBaseRoute.getAlarm().isAlarmEnabled()) {
					//cron에 의해 알림을 전송하는
					notificationService.saveMessage(latestBaseRoute); // FCM 메시지 전송
				}
