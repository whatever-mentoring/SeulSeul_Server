...
public class RouteDetailService {
	...
	public RouteDetailDto routeDetailFromBaseRoute(Long id) throws JsonProcessingException {
		...
		if (baseRoute.getExSID1() != null) {
			detailDto.updateFromBaseRoute(baseRoute.getFirstStation(), baseRoute.getLastStation(),
				baseRoute.getExName(), baseRoute.getExWalkTime(), baseRoute.getFastTrainDoor(), baseRoute.getLaneName(),
				baseRoute.getWayName(), baseRoute.getTravelTime());
		} else {
			detailDto.updateFromBaseRouteOnly(baseRoute.getFirstStation(), baseRoute.getLastStation(),
				baseRoute.getLaneName(), baseRoute.getWayName(), baseRoute.getTravelTime());
		}
		return detailDto;
	}

	@Transactional
	public List<String> compute(Long id) throws IOException {
			...
			if (resultTime.isEmpty()) {
				...
				while (true) {
					...
					if (h < hours || (h == hours && m <= minutes)) {
						...
						break;
					} else if (h == hours && m - 1 == minutes) { //내리는 경우 1분의 오차 존재 가능성 있음
						resultTime.add(timeList2[index]);
						break;
					} else {
						index -= 1;
	...
	@Transactional
	public RouteDetailDto updateTimeList(Long id, RouteDetailDto detailDto, String timeList) throws
