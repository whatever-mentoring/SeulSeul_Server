...
public class BaseRouteService {
	...
	// Odsay API 불러오기
	public String findUrl(double startX, double startY, double endX, double endY) throws IOException {
		String urlInfo =
			"https://api.odsay.com/v1/api/searchPubTransPathT?lang=0&SX=" + startX + "&SY=" + startY + "&EX="
				+ endX + "&EY=" + endY + "&SearchPathType=1&apiKey="
				+ URLEncoder.encode(apiKey.getApiKey(), "UTF-8");
		return urlInfo;
	}

	public String findJson(double startX, double startY, double endX, double endY) throws IOException {
		// http 연결
		String urlInfo = findUrl(startX, startY, endX, endY);
	...
	// 역 ID와 역 이름 가져오기
	@Transactional
	public BaseRoute findStationIdAndName(Long id) throws IOException {
		// baseRoute 객체 찾기
		BaseRoute baseRoute = baseRouteRepository.findById(id).orElseThrow(() -> new
			CustomException(ErrorCode.BASEROUTE_NOT_FOUND));

		// json 가져오기
		String jsonString = findJson(baseRoute.getStartX(), baseRoute.getStartY(), baseRoute.getEndX(),
			baseRoute.getEndY());
		// parsing
	...
	@Transactional
	public String findFromAPI(int SID, int EID) throws IOException {
		//1. API 연결
		String urlInfo =
			"https://api.odsay.com/v1/api/subwayPath?lang=0&CID=1000&SID=" + SID + "&EID=" + EID + "&Sopt=2&apiKey="
				+ URLEncoder.encode(apiKey.getApiKey(), "UTF-8");
	...
	public BaseRoute findTransferData(Long id) throws IOException {
		...
		String string = findFromAPI(baseRoute.getSID(), baseRoute.getEID());
		...
		try {
			...
			if (laneNameList.size() != 1) {
				...
				for (JsonNode stations : stationSetArray) {
					...
					if (exNameList.contains(stations.get("endName").asText()) || stations.get("endName")
						.asText()
						.equals(globalEndName)) {
			...
			if (laneNameList.size() != 1) {
				...
				baseRoute.update(StringLaneName, StringWayCode, StringWayName, StringExName, StringExSID1, StringExSID2,
					StringFastTrain, StringExWalkTime, StringTravelTime);
	...
	@Transactional
	public BaseRoute findBaseRouteByUser(User user) {
